...
class SlotAttention(nn.Module):
    ...
    def __init__(self, num_iterations, num_slots,
                 ...
                 epsilon=1e-8, use_detach=True):
        super().__init__()
        
        self.num_iterations = num_iterations -1 if use_detach else num_iterations
        ...
        self.iterate = self._iterate_det if use_detach else self._iterate_nodet
        
    def step(self, slots, B, N_q, N_kv, v, k):
        slots_prev = slots
    ...
    def _iterate_nodet(self, slots, B, N_q, N_kv, v, k):
        for _ in range(self.num_iterations):
            slots, attn_vis = self.step(slots, B, N_q, N_kv, v, k)
        return slots, attn_vis
    
    def _iterate_det(self, slots, B, N_q, N_kv, v, k):
        for _ in range(self.num_iterations):
            slots, attn_vis = self.step(slots, B, N_q, N_kv, v, k)
        slots, attn_vis = self.step(slots.detach(), B, N_q, N_kv, v, k)
        return slots, attn_vis
    ...
    def forward(self, inputs, slots):
        ...
        slots, attn_vis = self.iterate(slots, B, N_q, N_kv, v, k)
        return slots, attn_vis
...
class SlotAttentionEncoder(nn.Module):
    ...
    def __init__(self, num_iterations, num_slots,
                 input_channels, slot_size, mlp_hidden_size, num_heads, use_detach):
        super().__init__()
